...
class ChipsViewModel {
  ...
  getTestDriveChips(filtersData: FiltersData): Chip[] {
    const testDriveChips: Chip[] = [];
    const filtersDataTestDrive = filtersData[Filters.TEST_DRIVE];
    if (filtersDataTestDrive) {
      const matchingTestDrive = this.carsStore.testDrives.find(
        (testDrive) => testDrive.filtersDataValue === filtersDataTestDrive
      );
      if (matchingTestDrive) {
        testDriveChips.push({
          display: matchingTestDrive.display,
          handleDelete: () => {
  ...
  getChips(): Chip[] | undefined {
    ...
    const chips: Chip[] = [
      ...
      ...this.getTestDriveChips(filtersData),
