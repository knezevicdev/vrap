...
const Step = styled('div')(({theme})=> ({
    display: 'flex',
    alignItems: 'center',
    [theme.breakpoints.only('sm')]: {flexDirection: 'column', textAlign: 'center'},
    ...
    return (
        <PeaceOfMindContainer>
            <PeaceOfMindContainerContent>
                <Header variant="h2" fontWeight="fontWeightMedium">{viewModel.title}</Header>
                <Steps>
                    {viewModel.steps.map((step) => {
                        const {title, description, img: {alt, src}} = step;
                        return (
                            <Step key={title}>
