...
describe('Deal Validator', ()=> {
    ...
    describe("App Deal Validator",  () => {
        ...
        const fn = Promise.resolve(Deals[0]) as unknown as Promise<Response<Request.DealValidatorData>>
        spy.mockReturnValue(fn);

       const response = await initDealValidator(appContext as unknown as AppContext);
       ...
       expect(response.isDepositCaptured).toBeFalsy()
        ...
        it('Vehicle Sold', async() => { 
            ...
            //Mock API GraphQL Call
            const spy = jest.spyOn(Request, 'getDealValidator'); 
            const fn = Promise.resolve(Deals[1]) as unknown as Promise<Response<Request.DealValidatorData>>
            spy.mockReturnValue(fn);
        ...
        it('In-Progress Deal Deposit Captured', async() => { 
            ...
            //Mock API GraphQL Call
            const spy = jest.spyOn(Request, 'getDealValidator'); 
            const fn = Promise.resolve(Deals[2]) as unknown as Promise<Response<Request.DealValidatorData>>
            spy.mockReturnValue(fn);
        ...
        it('Pending Deal Deposit Captured', async() => { 
            ...
            //Mock API GraphQL Call
            const spy = jest.spyOn(Request, 'getDealValidator'); 
            const fn = Promise.resolve(Deals[3]) as unknown as Promise<Response<Request.DealValidatorData>>
            spy.mockReturnValue(fn);
        ...
        it('Unauthorized User', async() => { 
            ...
            //Mock API GraphQL Call
            const spy = jest.spyOn(Request, 'getDealValidator'); 
            const fn = Promise.resolve(Unauthorized) as unknown as Promise<ErrorResponse>
            spy.mockReturnValue(fn);
